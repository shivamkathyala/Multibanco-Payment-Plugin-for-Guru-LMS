        ...
        if (isset($mbKey, $orderId, $email, $amount)) {
            ...
            if (curl_errno($curl)) {
                ...
                if($message === "Success" && $status === "0"){
                    ...
                    // OUTPUT FORM
                    function buildForm($entity, $reference, $amt, $expiryDate = '', $includeLogo = true) {
                        ...
                        $form .= '<div id="multibanco-after-payment" class="multibanco-after-payment-cls" style="width: 100%; max-width: 740px; margin: 0 auto; border: 1px solid #0c629e; border-width: 4px 1px 1px 1px; padding-top: 60px;">';
                        $form .= '<div>';
                        ...
                        $form .= '</div>';
                        $form .= '<div class="inner_fields_text_multibanco" style="margin-bottom: 10px; border-bottom: 1px solid #ddd;">';
                        $form .= '<p style="font-size: 16px; color: #666;">' . Text::_('PLG_MB_PAY_REFERENCE') . '<span id="refrencia_multibanco" style="color: #666; font-weight: 700; font-size: 17px; float: right;">' . $reference . '</span></p>';
                        $form .= '</div>';
                        $form .= '<div class="inner_fields_text_multibanco" style="margin-bottom: 10px; border-bottom: 1px solid #ddd;">';
                    ...
                    $formWithLogo = buildForm($entity, $reference, $amt, $expiryDate, true);
                    $emailBody = buildForm($entity, $reference, $amt, $expiryDate, false);                    
